...
if (
  ...
  typeof process.env.LICENSE_KEY !== "undefined"
) {
...
if (
  typeof geodatadir === "undefined" &&
  typeof process.env.GEODATADIR !== "undefined"
) {
...
var databases = [
  {
    type: "country",
    url:
      "https://download.maxmind.com/app/geoip_download?edition_id=GeoLite2-Country-CSV&suffix=zip&" +
      license_key,
    checksum:
      "https://download.maxmind.com/app/geoip_download?edition_id=GeoLite2-Country-CSV&suffix=zip.sha256&" +
      license_key,
  ...
  },
  {
    type: "city",
    url:
      "https://download.maxmind.com/app/geoip_download?edition_id=GeoLite2-City-CSV&suffix=zip&" +
      license_key,
    checksum:
      "https://download.maxmind.com/app/geoip_download?edition_id=GeoLite2-City-CSV&suffix=zip.sha256&" +
      license_key,
  ...
  fs.readFile(
    ...
    function (err, data) {
      ...
      function onResponse(response) {
        ...
        if (status !== 200) {
          console.log(
            chalk.red("ERROR") + ": HTTP Request Failed [%d %s]",
            status,
        ...
        response.on("end", function () {
          if (str && str.length) {
            if (str == database.checkValue) {
              console.log(
                chalk.green('Database "' + database.type + '" is up to date')
              );
              database.skip = true;
            } else {
              console.log(
                chalk.green("Database " + database.type + " has new data")
              );
              database.checkValue = str;
            }
          } else {
            console.log(
              chalk.red("ERROR") + ": Could not retrieve checksum for",
              database.type,
              chalk.red("Aborting")
            );
        ...
        zipfile.on("entry", function (entry) {
          if (/\/$/.test(entry.fileName)) {
            ...
            zipfile.openReadStream(entry, function (err, readStream) {
              ...
              readStream.pipe(
                fs.createWriteStream(path.join(tmpPath, fileName))
              );
            });
          }
        });
...
function updateChecksum(database, cb) {
  ...
  fs.writeFile(
    ...
    function (err) {
      if (err)
        console.log(
          chalk.red("Failed to Update checksums."),
          "Database:",
          database.type
        );
      cb();

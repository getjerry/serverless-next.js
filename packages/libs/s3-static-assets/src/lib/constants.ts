export const IMMUTABLE_CACHE_CONTROL_HEADER =
  "public, max-age=31536000, immutable";

export const SERVER_CACHE_CONTROL_HEADER =
  "public, max-age=0, s-maxage=2678413, must-revalidate";

export const SERVER_CACHE_CONTROL_HEADER1 =
  "public, max-age=0, s-maxage=2678414, must-revalidate";

export const SERVER_CACHE_CONTROL_HEADER2 =
  "public, max-age=0, s-maxage=2678415, must-revalidate";

export const SERVER_CACHE_CONTROL_HEADER3 =
  "public, max-age=0, s-maxage=2678416, must-revalidate";

export const SERVER_CACHE_CONTROL_HEADER4 =
  "public, max-age=0, s-maxage=2678417, must-revalidate";

export const SERVER_CACHE_CONTROL_HEADER5 =
  "public, max-age=0, s-maxage=2678418, must-revalidate";

export const DEFAULT_PUBLIC_DIR_CACHE_CONTROL =
  "public, max-age=31536000, must-revalidate";

export const SERVER_NO_CACHE_CACHE_CONTROL_HEADER =
  "public, max-age=0, s-maxage=0, must-revalidate";

export const DEFAULT_PUBLIC_DIR_CACHE_REGEX =
  /\.(gif|jpe?g|jp2|tiff|png|webp|bmp|svg|ico)$/i;
